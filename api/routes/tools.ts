      ...
      case 'pdf-to-word': {
        ...
        try {
            ...
            sections: [{
              ...
              children: [
                new Paragraph({
                  children: [
                    new TextRun({
                      text: `Converted from PDF: ${inputFile.originalname}`,
                      bold: true,
                      size: 28
                    })
                  ]
          ...
          result = {
            ...
            fileName: outputFileName,
            fileSize: fs.statSync(outputPath).size,
            message: `Successfully converted PDF to ${outputFormat.toUpperCase()} with ${pdfData.numpages} pages`
          };
        } catch (error) {
          console.error('PDF to Word conversion error:', error);
          res.status(500).json({ 
            success: false, 
            error: 'Failed to convert PDF to Word. The PDF file may be corrupted or password-protected.' 
          });
          return;
        }
        break;
