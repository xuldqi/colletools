...
export default function Deadlines() {
  ...
  const addAssignment = () => {
    ...
    const newItem: Assignment = { id: Date.now(), title: newTitle, date: newDate, course: newCourse, completed: false, priority: newPriority, notes: newNotes }
  ...
  const toggleComplete = (id: number) => setAssignments(assignments.map((a) => (a.id === id ? { ...a, completed: !a.completed } : a)))
  const deleteAssignment = (id: number) => setAssignments(assignments.filter((a) => a.id !== id))
  ...
  return (
    <div className="max-w-3xl mx-auto py-10 px-4">
      ...
      <Card className="mb-8">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
          ...
          <div className="md:col-span-3">
            ...
            <DateInput value={newDate} onChange={setNewDate} className="w-full p-2 border rounded" />
          </div>
      ...
      <div className="space-y-3">
        ...
        {assignments.map((a) => (
          <div key={a.id} className={`border-l-4 p-4 rounded-r-lg shadow-sm flex justify-between items-center ${getUrgencyColor(a.date, a.completed)}`}>
            ...
            <div className="flex items-center gap-4 shrink-0">
              {!a.completed && <span className={`text-xs font-bold px-2 py-1 rounded ${new Date(a.date) < new Date() ? 'text-red-600 bg-red-100' : 'text-primary-600 bg-primary-50'}`}>{getDaysText(a.date)}</span>}
