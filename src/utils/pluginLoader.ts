...
export const PLUGINS: Record<string, PluginConfig> = {
  ffmpeg: {
    ...
    checkFunction: () => typeof (window as any).FFmpegWASM !== 'undefined' && (window as any).FFmpegWASM
  },
  ...
  'pdfjs-lib': {
    ...
    globalVar: 'pdfjsLib',
    checkFunction: () => typeof (window as any).pdfjsLib !== 'undefined'
  },
  ...
  tesseract: {
    ...
    globalVar: 'Tesseract',
    checkFunction: () => typeof (window as any).Tesseract !== 'undefined'
  },
  ...
  'pdf-lib': {
    ...
    globalVar: 'PDFLib',
    checkFunction: () => typeof (window as any).PDFLib !== 'undefined'
  }
};
...
class PluginManager {
  ...
  async loadPlugin(pluginName: string): Promise<void> {
    if (this.isPluginLoaded(pluginName)) return Promise.resolve();
    if (this.loadingPlugins.has(pluginName)) return this.loadingPlugins.get(pluginName)!;
  ...
  private async loadPluginInternal(plugin: PluginConfig): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onload = () => {
        ...
        setTimeout(() => {
          if (this.isPluginLoaded(plugin.name)) resolve();
          else reject(new Error('Plugin loaded but not available'));
        }, waitTime);
      };
...
export const createUsePluginLoader = (React: any) => {
  return (pluginName: string) => {
    const [loadingState, setLoadingState] = React.useState(() => 
      pluginManager.getLoadingState(pluginName) || null
    );
